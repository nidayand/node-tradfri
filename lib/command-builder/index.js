...
class CoapCommandBuilder {
    ...
    get(type, id) {
        ...
        let command = `${this.coapClientPath} -m get -u "${this.username}" -k "${this.securityId}" "coaps://${this.ip}:5684/${endpoint}`;
    ...
    put(type, id, operation) {
        ...
        const command = `${this.coapClientPath} -m put -u "${this.username}" -k "${this.securityId}" -e ${e} "coaps://${this.ip}:5684/${endpoint}/${id}"`;

        return command;
    }

    static create(coapClientPath, securityId, hubIpAddress) {
        return new CoapCommandBuilder(coapClientPath, username, securityId, hubIpAddress);
    }
}
